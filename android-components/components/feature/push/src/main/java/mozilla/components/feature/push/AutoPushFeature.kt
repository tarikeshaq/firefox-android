...
class AutoPushFeature(
    ...
    private fun exceptionHandler(onError: (PushError) -> Unit) = CoroutineExceptionHandler { _, e ->
        when (e) {
            ...
            is UaidNotRecognizedException -> verifyActiveSubscriptions(forceVerify = true)
            else -> onError(PushError.Rust(e, e.message.orEmpty()))
        }
    }
...
fun Protocol.toRustHttpProtocol(): PushHttpProtocol {
    return when(this) {
